	...
	$.fn.mcDropdown = function(list, options) {
		...
		this.each(function() {
			...
			if (className==undefined){
				className='mcDropdown';
//$('#cmpItem1').html("empty");
//			}else {
//$('#cmpItem1').html("notEmpty-> "+className);
			}						
//$('#cmpItem2').html(className);			
			new $.mcDropdownMenu(this, list, options, className);
		});
//$('#cmpItem3').html(className);				
	...
	var defaults = {
		...
		, allowParentSelect: true     // determines if parent items are allowed to be selected (by default only end nodes can be selected)
	...
	browser.isIE = (jQBrowser.msie || (/\sTrident\/\d/i).test(window.navigator.userAgent));
	browser.isIE6 = (browser.isIE && jQBrowser.version && jQBrowser.version <= 6);

	$.mcDropdownMenu = function(el, list, options, className){
...
//$('#cmpItem3').html('ttt  '+className);		
		...
		if( settings.click == null ) {
			settings.click = function (e, dropdown, settings){
				if( this.prop(settings.valueAttr) ){
					dropdown.setValue(this.prop(settings.valueAttr));
				} else {
					dropdown.setValue($(this.parents("li")[0]).prop(settings.valueAttr));
				}
			};
		}
		...
		var $input = $self.replaceWith($divInput).prop({id: "", name: ""});
		...
		// IE doesn't register certain keypress events, so we must catch them during the keydown event
		if( browser.isIE || jQBrowser.safari) $self.bind("keydown", function (e){
			// check to see if a key was pressed that IE doesn't trigger a keypress event for
		...
		this.setValue = function (value, skipCallback){
			...
			$hidden.val(value);
			//فراخوانی تریگر تغییر صفحه فیلدهای اضافی در صفحه افزودن کالاها
			$("#category").trigger("change_macdropdown");
		...
		this.openMenu = function (e){
			...
			function open(){
				...
				if( browser.isIE6 && !!$.fn.bgIframe ) $list.bgIframe();
			}
...
*/						
			...
			if( browser.isIE6 ) e.stopPropagation();
	  };
		...
		function hideBranch(force){
			...
			if( browser.isIE6 ) $ul.css("visibility", "hidden").parent().removeClass("mc_hover");
...
/** 			
$('#cmpItem1').html('pos.top-> ' + pos.top + ' ,pos.bottom->' + pos.bottom + ' ,pos.left->' + pos.left + ' ,pos.right->' + pos.right);
 **/	
...
/** 
$('#cmpItem2').html('pos.top-> ' + pos.top + ' ,pos.bottom->' + pos.bottom + ' ,pos.left->' + pos.left + ' ,pos.right->' + pos.right);
 **/
			/*
...
//$('#cmpItem1').html('ddWidth-> ' + ddWidth);
//$('#cmpItem2').html('width-> ' + width);
//$('#cmpItem3').html('el.data("width")->' + $el.data("width"));
//$('#cmpItem3').html('el.data("dimensions")->' + $el.data("dimensions"));
//$('#cmpItem3').html('cols-> ' + cols);
//$('#cmpItem3').html('Math.floor(ddWidth/cols)-> ' + Math.floor(ddWidth/cols));
//$('#cmpItem3').html('($el === $list) && ($el.data("width") * cols < ddWidth)-> ' + (($el === $list) && ($el.data("width") * cols < ddWidth)));
		...
		function onFocus(e){
			...
			if( !$keylist ){
				...
				if( browser.isIE6 && !!$.fn.bgIframe ) $keylistiframe = $('<div></div>').bgIframe().appendTo("body");
			} 
		...
		function onBlur(e){
			...
			iBlurTimeout = setTimeout(function (){
				...
				if( !settings.allowParentSelect && $current.is(".mc_parent") ){
					...
					var value = $current.find("li:not('.mc_parent'):first").prop(settings.valueAttr);
					// update the value
		...
		function showMatches($li, hideResults){
			...
			if( hideResults === true ){
				...
				if( browser.isIE6 && !!$.fn.bgIframe ) $keylistiframe.css("display", "none");
			}
		...
		function hideMatches(){
			// hide the bgiframe
			if( browser.isIE6 && !!$.fn.bgIframe && $keylistiframe ) $keylistiframe.hide();
		...
		function updateValue($li, keepTypedText){
			...
			thismenu.setValue($li.prop(settings.valueAttr), true);
