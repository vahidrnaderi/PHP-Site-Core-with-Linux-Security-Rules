...
(function($) {
	var NivoSlider = function(element, options) {
		...
		if (settings.directionNav) {
			slider
					.append('<div class="nivo-directionNav"><span class="nivo-prevNav">'
							+ settings.prevText
							+ '</span><span class="nivo-nextNav">'
							+ settings.nextText + '</span></div>');
			...
			$('span.nivo-prevNav', slider).live('click', function() {
				if (vars.running)
					return false;
				clearInterval(timer);
				timer = '';
			...
			$('span.nivo-nextNav', slider).live('click', function() {
				if (vars.running)
					return false;
				clearInterval(timer);
				timer = '';
	...
	$.fn.nivoSlider.defaults = {
		...
		controlNavThumbsSearch : 'images',
		controlNavThumbsReplace : '_thumbs/Images',
		keyboardNav : true,
		pauseOnHover : true,
		manualAdvance : false,
		captionOpacity : 0,
