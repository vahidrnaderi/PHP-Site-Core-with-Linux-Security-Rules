...
class chart{
	...
	function MakeLinePointChart($fileName)
	{
		...
		if ($this->Title)
		{
			...
			$font = 'theme/threeColumns/font/FreeFarsi.ttf';
			$this->Title = fagd($this->Title,'fa');
			$charty += $ifh1 + 2;
			################## Start Faragaman Logo ##################
			imagettftext($im, 9, 0, ($this->Width - $len)-160, $charty, $orange, $font, "Created by faragamane-sharif company");
			//imagettftext($im, 8, 0, ($this->Width - $len)-99, $charty-=1, $grey, $font, "Created by faragamane-sharif company");
			################### End Faragaman Logo ###################
			imagettftext($im, 10, 0, ($this->Width - $len)+200, $charty, $black, $font, $this->Title);
			imagettftext($im, 10, 0, ($this->Width - $len)+199, $charty-=1, $grey, $font, $this->Title);

			//ImageLine($im, $this->Width - $len, $charty, ($this->Width + $len) / 4, $charty, $black);
		...
		if ($this->SubTitle)
		{
			...
			$this->SubTitle = fagd($this->SubTitle,'fa');
			imagettftext($im, 10, 0, ($this->Width - $len) / 2, $charty, $black, $font, $this->SubTitle);
			imagettftext($im, 10, 0, (($this->Width - $len) / 2) - 1, $charty-=1, $grey, $font, $this->SubTitle);
		...
		if ($this->xCount)
		{
			...
			for ($i = 0; $i < $this->xCount; $i++)
			{
				...
				{
					if ($this->xValue[0][$i]){
						$this->xValue[0][$i] = fagd($this->xValue[0][$i],'fa');
						imagettftext($im, 8, 270, $xoff, $charth + (strlen($this->xValue[0][$i])) + 20, $colors[0], $font, $this->xValue[0][$i]);

						//ImageStringUp($im, $font3, $xoff, $charty + $charth + 5 + (strlen($this->xValue[0][$i]))*$ifw3, $this->xValue[0][$i], $colors[0]);
					}
					if ($this->xValue[1][$i]){
						$this->xValue[1][$i] = fagd($this->xValue[1][$i],'fa');
						imagettftext($im, 10, 90, $xoff, $charty + $charth + 5 + (strlen($this->xValue[1][$i]))*$ifw3, $colors[1], $font, $this->xValue[1][$i]);
		...
		ImagePNG($im, "home/$fileName/resultChart.png");
		ImageDestroy($im);
	}
	...
	public function chartMaker($xArray, $yArray, $fileName, $title=null, $subTitle=null, $height=400){
		...
		$xcount = 0;
		foreach ($xArray as $xItem)
		$xcount = max($xcount, count($xItem));

		#BIGGEST Y ELEMENT########################
		$ymax = 0;
		foreach ($yArray as $yItem)
		$ymax = max($ymax,ceil(max($yItem)));

		#POPULATING GRAPH#########################
		$this->xValue = array();
		$this->DataValue = array();

		foreach ($yArray as $k => $yItem){
			foreach ($yItem as $kk => $yIt)
			{
				$this->xValue[$k][] = $xArray[$k][$kk];
				$this->DataValue[$k][] = $yIt;
			}
		}
		...
		$this->Width = ($xcount*45) + 150;
		$this->Height = $height;
		$this->ShowBullets = TRUE;

		$this->LineShowCaption = FALSE; // TO BE FIXED YET
		$this->LineShowTotal = FALSE;   // DEPENDS ON LineShowCaption to be TRUE
		$this->LineCaption[0] = "Period 1";
		$this->LineCaption[1] = "Period 2";
		$this->LineCount = 2;

		$this->xCount = $xcount;
		$this->xCaption = "";
		$this->xShowValue = TRUE;
		$this->xShowGrid = TRUE;

		$this->yCount = 10;
		$this->yCaption = "";
		$this->yShowValue = TRUE;
		$this->yShowGrid = TRUE;

		$this->DataDecimalPlaces = 0;
		$this->DataMax = $ymax;
		$this->DataMin = 0;
		$this->DataShowValue = TRUE;

		#ITS DRAWING TIME###########################
		$this->MakeLinePointChart($fileName);
	}
