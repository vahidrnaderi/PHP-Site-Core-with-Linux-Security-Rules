		...
		if($system->dbm->db->count_records("`$this->userTable`", "`email` = '$values[email]'") == 0){
			...
			if(strlen($values['password']) >= $settings['minCharPassword']){
				if($_POST['securityQuestion'] === $system->dbm->db->informer("`$settings[faqObject]`", "`id` = $_POST[securityId]", "answer")){
					...
					$system->dbm->db->insert("`$this->userTable`", "`active`, `timeStamp`, `owner`, `group`, `or`, `ow`, `ox`, `gr`, `gx`, `tr`, `tx`, `gid`, `password`, `email`, `userType`", "1, '$timeStamp', 1, 1, 1, 1, 1, 1, 1, 1, 1, 2, '$password', '$values[email]', '$values[userType]'");
...
//		echo ("<br>*** " . $system->dbm->db->informer("`$settings[userTable]`", "`id` = $_SESSION[uid]", "mobile"))."<br> != <br>**** " .$values['mobile'] ;
		if($values['userType'] == 2){  // company Type
//			echo 2;
